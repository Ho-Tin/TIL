...
- 다중 상속
  ...
  - MRO가 필요한 이유
    - 부모 클래스들이 여러 번 액세스 되지않도록 한번만 호출
    - 프로그래밍 언어의 신뢰성 있고 확장성 있는 클래스 설계할수 있도록 도움
    - 예측 가능해지며, 코드의 재사용이나 유지보수 가능하다
   
  - 클래스를 배우는 이유
    - 작은 프로젝트는 변수와 함수만으로 간단한 프로그램을 만들수 있다.
    - ㅋ하지만 규모가 커지면 관리하기 어려움
    - 클래스를 사용하면 **관련된 데이터와 기능**하나로 묶어 관리 가능
### 에러와 예외
  - 버그(bug)
  - 디버깅(Debugging)
    - 버그를 찾아내고 제거하는 작업
  - 에러
    - 문법 에러 : 구문이 올바르지 않을때 나오는 Error
   
  - 예외
- try-except 구조
  - try 블록 안에는 예외가 발생할 수 있는 코드를 작성
- 복수 예외 처리
- 에러 조건을 안쓰고 except: 바로하면 모든 에러

- else & finally
  - else 블록은 예외가 발생하지 않았을 떄 추가 작업을 진행
  - as 키워
## 250806
- 2차원 리스트
  - 리스트 안의 리스트  list[[1,2,3,4][1,2,3,4]] / list[0][0] = 1
  - n*m (행 * 열)
  - 행 우선 순위 : 행을 고정하고 열을 움직임
  - 열 우선 순위 : 열을 고정하고 행을 움직임
  - 지그재그 우선순위 - i,j 중 i는 고정 j를 바꾸는대 짝수일때는 정방향 홀수일때 역방향 (**m-1-j**)로 접근[i][j - (m-1-(2*j) *(j%2))]
### 델타 탐색 : 이웃한 요소
- 2차 리스트에서 4방향의 인접 요소로 탐색하는 방법
- 상 하 좌 우 로 움직여서 찾는것
```
dx = [0,0,1,-1]
dy = [1,-1,0,0]
for i in range(N):
  for j in range(N):
    abs_sum = 0 # 델타탐색 끝나면 초기화 (합계를 더하는 문제)
    for k in ranbe(4) # 델타탐색
      ni = i + dx[k]
      nj = j + dy[k]
